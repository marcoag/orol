...
macro(DISSECT_VERSION)
    ...
    string(REGEX REPLACE "^([0-9]+).*" "\\1"
        OROL_MAJOR_VERSION "${OROL_VERSION}")
    string(REGEX REPLACE "^[0-9]+\\.([0-9]+).*" "\\1"
        OROL_MINOR_VERSION "${OROL_VERSION}")
    string(REGEX REPLACE "^[0-9]+\\.[0-9]+\\.([0-9]+).*" "\\1"
        OROL_REVISION_VERSION "${OROL_VERSION}")
    string(REGEX REPLACE "^[0-9]+\\.[0-9]+\\.[0-9]+(.*)" "\\1"
        OROL_CANDIDATE_VERSION "${OROL_VERSION}")
...
macro(SET_INSTALL_DIRS)
  if (NOT DEFINED LIB_INSTALL_DIR)
    ...
    set(INCLUDE_INSTALL_ROOT
        "include/${PROJECT_NAME_LOWER}-${OROL_MAJOR_VERSION}.${OROL_MINOR_VERSION}")
    set(INCLUDE_INSTALL_DIR "${INCLUDE_INSTALL_ROOT}/orol")
    set(DOC_INSTALL_DIR "share/doc/${PROJECT_NAME_LOWER}-${OROL_MAJOR_VERSION}.${OROL_MINOR_VERSION}")
    set(BIN_INSTALL_DIR "bin")
    set(PKGCFG_INSTALL_DIR "${LIB_INSTALL_DIR}/pkgconfig")
    if(WIN32)
        set(OROLCONFIG_INSTALL_DIR "cmake")
    else(WIN32)
        set(OROLCONFIG_INSTALL_DIR "share/${PROJECT_NAME_LOWER}-${OROL_MAJOR_VERSION}.${OROL_MINOR_VERSION}")
    endif(WIN32)
